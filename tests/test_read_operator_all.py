...
class Test_Enter(LoginPage):
    login = LoginPage()
    login.login_as('inv_operator_all_pechora', '1')
...
@allure.feature("Проверка пользователя с ролью оператор")
@allure.story("Нет кнопок редактировать/создать. Нет меню, к которым нет доступа")
